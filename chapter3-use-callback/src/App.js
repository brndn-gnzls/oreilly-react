...
//         setSpeakers(speakers.concat({
//             id: uuid(),
//             name: text
//         }))
//     }

//     // useCallback, the code is similar to useMemo except that useCallback
//     // is wrapped around the function we intend to cache or memoize.
//     const handleRemoveSpeaker = useCallback(
//         (id) => setSpeakers(speakers.filter((user) => user.id !== id)), [speakers]
//     )

//     return (
//         <div>
//             <input type="text" value={text} onChange={handleTextInput} />
//             <button type="button" onClick={handleAddSpeaker}>+ Add a Speaker</button>
...
const App = () => {
    ...
    const handleRemoveSpeaker = useCallback(
        (id) => setSpeakers((prevSpeakers) => prevSpeakers.filter((user) => user.id !== id)), []
    );
